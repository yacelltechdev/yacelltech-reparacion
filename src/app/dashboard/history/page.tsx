          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="text-[11px] text-slate-400 mt-0.5 space-y-0.5">
                <div>Ingreso: {formatDate(repair.fecha)}</div>
                {repair.fecha_entrega_recepcion && (
                  <div className="text-violet-700">Recepción: {formatDate(repair.fecha_entrega_recepcion)}</div>
                )}
                {repair.fecha_despacho && <div>Despachado: {formatDate(repair.fecha_despacho)}</div>}
              </div>
            </div>
